...
def post_prune(tree, X_val, y_val, X_train, y_train, X_test, y_test,
    ...
    print("Starting Post-pruning with Gini impurity...")
...
def run_all_curves(train_path, val_path, test_path, output_folder):
    ...
    for max_depth in depths:
        print(f"Training DecisionTree with Gini impurity, max_depth={max_depth}...")
        ...
        plt.ylabel("Accuracy")
        plt.title(f"Gini Post-Pruning Curve (max_depth={max_depth})")
        plt.legend(loc='upper right')
        plt.tight_layout()
        outfile = os.path.join(output_folder, f"gini_post_prune_curve_depth{max_depth}.png")
        plt.savefig(outfile)
        plt.close()
        print(f"Plot saved as {outfile}")

    print("All separate Gini post-pruning plots saved to the output folder.")

if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python d.py <train_csv> <val_csv> <test_csv> <output_folder_path>")
        sys.exit(1)
