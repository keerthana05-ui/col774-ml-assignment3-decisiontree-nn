...
def main(train_dir, test_dir, output_dir):
    ...
    for arch in hidden_archs:
        depth = len(arch)
        print(f"\nTraining network with depth {depth}, hidden layers {arch}")
        nn = NeuralNetwork(n_features, arch, n_classes,
    ...
    best_idx = np.argmax(avg_f1_scores)   # Save predictions of best model (highest avg F1)
    best_arch = hidden_archs[best_idx]
    print(f"\nRetraining best model with hidden layers {best_arch} for final prediction...")

    nn = NeuralNetwork(n_features, best_arch, n_classes,
                       learning_rate=learning_rate, batch_size=batch_size)
    nn.fit(X_train, y_train_oh, epochs=epochs)

    best_test_preds = nn.predict(X_test)

    output_file = os.path.join(output_dir, 'prediction_c.csv')
    ...
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'f1_vs_network_depth.png'))
    plt.show()
